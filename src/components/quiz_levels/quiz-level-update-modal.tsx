...
import { handleUpdateQuizLevelAction } from "@/utils/action"
...
const QuizLevelUpdateModal = (props: IProps) => {
  ...
  useEffect(() => {
    if (dataUpdate) {
      ...
      setFormData({
        ...
        description: dataUpdate.description ? (dataUpdate.description.trim() ?? "") : "",
      })
    }
  }, [dataUpdate])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        const res = await handleUpdateQuizLevelAction({
          id: dataUpdate.id,
          name,
          description,
        });
        if (res?.data) {
          toast("Level has been updated successfully.");
          handleCloseUpdateModal();
        } else {
          toast(res?.message);
        }
      } catch (error) {
        toast("Could not update level. Please try again.");
      }
    };
  ...
  return (
    <Dialog open={isUpdateModalOpen} onOpenChange={(isOpen) => isOpen || handleCloseUpdateModal()}>
      ...
      >
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">Update Level</DialogTitle>
          <DialogDescription className="text-gray-500 dark:text-gray-400">
            Update level information below.
          </DialogDescription>
        </DialogHeader>
...
export default QuizLevelUpdateModal;

...
import {
  handleGetCategoriesQuiz,
  handleCreateCategoryQuiz,
  handleDeleteCategoryQuiz,
  handleUpdateCategoryQuiz,
} from "@/utils/action";
...
export default function CategoryQuizPage() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        setIsLoading(true);
        const items = await handleGetCategoriesQuiz();
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const response = await handleCreateCategoryQuiz({
        name: name,
  ...
  const handleUpdate = async () => {
    ...
    try {
      const { name, description, slug, parent_id } = formData;
      const response = await handleUpdateCategoryQuiz(editingCategory, {
        name: name,
        description: description,
        slug: slug,
  ...
  const confirmDelete = async () => {
    if (categoryToDelete) {
      ...
      try {
        const response = await handleDeleteCategoryQuiz(categoryToDelete);

...
export function PostEditor({
  ...
  const updateBlockContent = useCallback(
    (id: string, content: string | File, previewUrl = "") => {
      updateBlocks((prevBlocks) =>
        prevBlocks.map((block) => (block.id === id ? { ...block, content, previewUrl } : block)),
      )
    },
    [updateBlocks],
  )
  ...
  const getPreviewBlocks = useCallback(() => {
    return blocks.map((block) => ({
      ...block,
      content: block.content instanceof File ? URL.createObjectURL(block.content) : (block.content as string),
    }))
  }, [blocks])
...
function renderBlockContent(
  ...
  setShowMathToolbar: (show: boolean) => void,
) {
  switch (block.type) {
    case "text":
      return (
        <Tiptap
          ...
          setShowMathToolbar={setShowMathToolbar}
        />
      )
    case "image":
      return (
        <div className="space-y-2">
          ...
          {block.content && (
            <div className="mt-2 rounded-md overflow-hidden">
              <img
                src={block.content instanceof File ? URL.createObjectURL(block.content) : block.content}
                alt="Content"
                className="max-h-[200px] object-contain"
                onError={(e) => {
                  e.currentTarget.src = "/placeholder.svg?height=200&width=400"
      ...
      return (
        <div className="space-y-2">
          ...
          <FileUploadArea
            onFileSelect={onFileSelect}
            onFileUpload={onFileUpload}
            value={typeof block.content === "string" && block.content.startsWith("blob:") ? block.content : undefined}
            accept="video/*"
          />
        </div>
      )
    ...
    case "list":
      return <Tiptap content={block.content as string} onChange={(content) => updateContent(block.id, content)} showMathToolbar={showMathToolbar} setShowMathToolbar={setShowMathToolbar} />
    default:
      return null
  }
}

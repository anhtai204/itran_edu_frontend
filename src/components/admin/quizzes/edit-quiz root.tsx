              ...
              const mappedQuestions = fetchedQuiz.questions.map((q: any) => ({
                ...
                content:
                  ...
                  q.question_type === "multiple-choice"
                    ...
                    ? {
                        pairs: q.content.pairs.map((pair: any) => ({
                          image_url: pair.image_url,
                          preview_url: pair.image_url,
                          label: pair.label,
                        })),
                      }
                    : q.content,
  ...
  const handleAddQuestion = async () => {
    ...
    try {
      ...
      if (newQuestion.type === "image-matching") {
        const pairs = (newQuestion.content as { pairs: { image_file?: File; image_url?: string; label: string }[] }).pairs;
        const arrayImages = pairs.map((pair) => pair.image_file).filter((file) => file !== undefined) as File[];
        const res = await uploadManyFileToServer(arrayImages);

        console.log('>>>res: ', res);
        finalContent = {
          pairs: pairs.map((pair, index) => ({
        image_url: pair.image_file ? res.data.urls[index] : pair.image_url,
        label: pair.label,
          })),
        };
      }
      ...
      switch (newQuestion.type) {
        ...
        case "matching":
          const matchingPairs = (
            newQuestion.content as { pairs: { item: string; match: string }[] }
          ).pairs;
          if (
            matchingPairs.some(
            ...
            toast("All matching items must have text");
            setIsLoading(false);
            return;
          }
          break;
        case "image-matching":
          const imagePairs = (
            newQuestion.content as {
              pairs: { image_url: string; label: string }[];
            }
          ).pairs;
          if (imagePairs.some((pair) => !pair.label.trim())) {
            toast("All images must have labels");
            setIsLoading(false);
            return;
          }
          break;
      ...
      if (editingQuestionId && !isTemporaryQuestion) {
        ...
        console.log(">>>res update ques: ", res);
        ...
        // console.log('>>>question data create question: ', questionData);
        // console.log('>>>res create question: ', res);

        if (res.statusCode === 201) {
          const question = res.data;

          // Replace the temporary question with the real one from the server
                                      ...
                                      {newQuestion.type === "matching" && (
                                        <div className="space-y-4">
                                          ...
                                          <p className="text-sm text-muted-foreground">
                                            Create pairs of items that students
                                            will need to match correctly.
                                          </p>
                                          {(
                                            newQuestion.content as {
                                              pairs: {
                                            ...
                                            return pairs.map((pair: any, index: number) => (
                                              ...
                                              >
                                                <div className="w-24 h-24 border border-muted rounded flex items-center justify-center relative bg-muted/10">
                                                  <img
                                                    src={pair.image_url ? `${process.env.NEXT_PUBLIC_BACKEND_URL}/${pair.image_url}` :
                                                      pair.preview_url || "/placeholder.svg"}
                                                    alt={`Image ${index + 1}`}
                                                ...
                                                <Input
                                                  ...
                                                  onChange={(e) => {
                                                    const updatedPairs = [...pairs];
                                                    updatedPairs[index].label = e.target.value;
                                                    setNewQuestion({
                                                      ...newQuestion,
                                                      content: { pairs: updatedPairs },
                                                    });

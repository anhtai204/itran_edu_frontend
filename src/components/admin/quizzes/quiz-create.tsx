...
export default function CreateQuizPage(props: any) {
  ...
  // Kiểm tra xem có thay đổi chưa lưu
  const hasUnsavedChanges = () => {
    // So sánh quiz với trạng thái ban đầu
    const quizChanged = Object.keys(initialQuiz).some(
      (key) =>
        quiz[key as keyof typeof quiz] !== initialQuiz[key as keyof typeof initialQuiz]
    );
    // Kiểm tra xem có câu hỏi nào hay không
    const questionsChanged = questions.length > 0;
    return quizChanged || questionsChanged;
  };

  // Xử lý sự kiện beforeunload cho reload/đóng tab
  useEffect(() => {
    const handleBeforeUnload = (event: BeforeUnloadEvent) => {
      if (hasUnsavedChanges()) {
        event.preventDefault();
        event.returnValue =
          "Bạn có chắc muốn thực hiện thao tác? Hành động không thể hoàn tác";
      }
    };
  ...
  // Xử lý sự kiện popstate cho hành động quay lại
  useEffect(() => {
    const handlePopState = (event: PopStateEvent) => {
      if (hasUnsavedChanges()) {
        event.preventDefault();
        setShowBackConfirm(true);
        // Đẩy lại lịch sử để ngăn quay lại ngay lập tức
  ...
  // Clean up when quiz is published or saved as draft
  ...
  const handleEditQuestion = (question: any) => {
    ...
    // Initialize default content based on question type if content is undefined or invalid
    ...
    if (!question.content || typeof question.content !== "object") {
      switch (question.type) {
        ...
        case "image-matching":
          content = {
            ...
            preview_urls: ["/placeholder.svg", "/placeholder.svg"],
          };
          break;
        case "fill-blanks":
          // Initialize based on question.text if available
          const tempDiv = document.createElement("div");
          tempDiv.innerHTML = question.text || "";
          const textContent = tempDiv.textContent || "";
          const blanks = Array.from(textContent.matchAll(/\[\[(.*?)\]\]/g)).map((match) => ({
            id: uuidv4(),
            text: match[1],
          }));
          content = {
      ...
      try {
        ...
        content = JSON.parse(JSON.stringify(question.content));
        // For fill-blanks, validate and sync with question.text
        if (question.type === "fill-blanks") {
          ...
          // Type guard to ensure content is of fill-blanks type
          const fillBlanksContent = content as {
            answers: { id: string; text: string }[];
            correct_answers: string[];
          };
          if (
            !fillBlanksContent.answers ||
            fillBlanksContent.answers.length !== blanks.length ||
            fillBlanksContent.answers.some((ans, i) => ans.text !== blanks[i].text)
          ) {
            content = {
              answers: blanks,
              correct_answers: blanks.map((answer) => answer.id),
        ...
        // Fallback to default content for the question type
        switch (question.type) {
          case "single-choice":
          case "multiple-choice":
            content = {
    ...
    // Set math toolbar states
    setMathToolbarStates({
      description: false,
      questionText: true,
      explanation: true,
      ...Object.fromEntries(
        (content as any).options?.map((opt: any) => [
          `option-${opt.id}`,
          true,
        ]) || []
      ),
  ...
  const handleAddQuestion = async () => {
    ...
    try {
      ...
      // Xử lý câu hỏi dạng fill-blanks
      if (newQuestion.type === "fill-blanks") {
        const content = newQuestion.content as { answers: { id: string; text: string }[], correct_answers: string[] };

        const answers = content.answers;
        const correct_answers = content.correct_answers;
        ...
        // Lọc bỏ các câu trả lời rỗng
        const validAnswers = content.answers.filter((answer) => answer.text.trim());

        if (validAnswers.length === 0) {
          toast("At least one valid answer is required");
          setIsLoading(false);
          return;
        }
        ...
        finalContent = {
          ...
          correct_answers: validAnswers.map(answer => answer.id),
        };
      }
      ...
      switch (newQuestion.type) {
        ...
        case "true-false":
          ...
          break;
        case "fill-blanks":
          // Validation đã được xử lý ở trên
          break;
      }
      ...
      const questionData = {
        ...
        question_order: editingQuestionId
          ? questions.find((q) => q.id === editingQuestionId)?.question_order || questions.length + 1
          : questions.length + 1,
      };
      ...
      toast(editingQuestionId ? "Failed to update question" : "Failed to add question");
    } finally {
      setIsLoading(false);
    }
  };
                            ...
                            >
                              {(provided) => (
                                ...
                                >
                                  ...
                                  {editingQuestionId === question.id && showAddQuestion && (
                                      ...
                                      {newQuestion.type === "fill-blanks" && (
                                        <div className="space-y-4">
                                          ...
                                          <div>
                                            ...
                                            {(newQuestion.content as { answers: { id: string; text: string }[] }).answers
                                              .length === 0 && (

...
import ai1 from "../../../assets/images/ai.webp";
...
type QuizQuestionContent =
  ...
  | { answers: { id: string; text: string }[] }; // fill-blanks
...
export default function EditQuizPage(props: any) {
                    ...
                    : q.question_type === "matching"
                      ...
                      : q.question_type === "image-matching"
                        ? {
                          ...
                          preview_urls: q.content.image_urls.map(
                            (img: any) => `${process.env.NEXT_PUBLIC_BACKEND_URL}/${img.url}`
                          ),
                        }
  ...
  // Add new useEffect for handling question updates
  useEffect(() => {
    ...
    try {
      ...
      if (newQuestion.type === "image-matching") {
        ...
        if (imageFiles.length > 0) {
          ...
          finalContent = {
            ...
            correct_image_matches: content.correct_image_matches,
          };
        } else {
          finalContent = {
            labels: content.labels,
      ...
      switch (newQuestion.type) {
        ...
        case "matching":
          ...
          if (items.some((item: any) => !item.text.trim()) || matches.some((match: any) => !match.text.trim())) {
            toast("All items and matches must have text");
            setIsLoading(false);
            return;
          }
          if (correct_matches.length !== items.length || correct_matches.length !== matches.length) {
          ...
          break;
        case "image-matching":
          const { labels, image_urls, correct_image_matches } = newQuestion.content as any;
          if (labels.some((label: any) => !label.text.trim())) {
            toast("All labels must have text");
            setIsLoading(false);
            return;
          }
          if (image_urls.some((img: any) => !img.url && !(newQuestion.content as any).image_files?.length)) {
            toast("All images must be uploaded");
            setIsLoading(false);
            return;
          }
          if (correct_image_matches.length !== labels.length || correct_image_matches.length !== image_urls.length) {
            toast("Each label must be matched with an image");
            setIsLoading(false);
            return;
          }
          break;
              ...
              <div className="space-y-2">
                ...
                <div className="border rounded-md shadow-sm">
                  <Tiptap
                    ...
                    onChange={(content) =>
                      handleQuizChange("description", content)
                    }
                                      ...
                                      <div className="space-y-2">
                                        ...
                                        <div className="border rounded-md shadow-sm">
                                          <Tiptap
                                            ...
                                            showMathToolbar={
                                              mathToolbarStates.questionText
                                            }
                                            setShowMathToolbar={() =>
                                              toggleMathToolbar("questionText")
                                            }
                                          />
                                        </div>
                                        {newQuestion.type === "fill-blanks" && (
                                          <p className="text-xs text-muted-foreground">
                                            Use double square brackets to create
                                            blanks: [[answer]]. For example:
                                        ...
                                        <div className="space-y-4">
                                          ...
                                          {(
                                            ...
                                            <div key={item.id} className="flex items-center gap-2">
                                              ...
                                              >
                                                <SelectTrigger>
                                                  <SelectValue placeholder={`Select match for item ${index + 1}`} />
                                                </SelectTrigger>
                                                <SelectContent>
                                                  {(newQuestion.content as any).matches.map((match: any) => (
                                                    <SelectItem key={match.id} value={match.id}>
                                                      {match.text || `Match ${match.id}`}
                                                    </SelectItem>
                                                  ))}
                                                </SelectContent>
                                              </Select>
                                          ...
                                          <div className="space-y-2">
                                            <Label>Matches</Label>
                                            {(newQuestion.content as any).matches.map((match: any, index: number) => (
                                              <div key={match.id} className="flex items-center gap-2">
                                                <Input
                                                  ...
                                                  onChange={(e) => {
                                                    ...
                                                    updatedMatches[index].text = e.target.value;
                                                    setNewQuestion({
                                                      ...newQuestion,
                                                      content: {
                                                        ...(newQuestion.content as any),
                                                ...
                                                />
                                                {(newQuestion.content as any).matches.length > 2 && (
                                                  <Button
                                                    variant="ghost"
                                                    size="icon"
                                                    onClick={() => {
                                                      ...
                                                      const updatedCorrectMatches = [
                                                        ...(newQuestion.content as any).correct_matches,
                                                      ].filter((cm: any) => cm.match_id !== match.id);
                                                      updatedMatches.splice(index, 1);
                                                      setNewQuestion({
                                                        ...newQuestion,
                                                        content: {
                                          ...
                                          {(() => {
                                            ...
                                            return labels.map((label: any, index: number) => (
                                              <div key={label.id} className="flex items-center gap-3">
                                                    ...
                                                    <FileUploadArea
                                                      onFileSelect={(file: File, previewUrl: string) => {
                                                        ...
                                                        if (!updatedImageUrls[index]) {
                                                          updatedImageUrls[index] = { id: `img-${Date.now()}`, url: "" };
                                                        }
                                                        setNewQuestion({
                                                          ...newQuestion,
                                                          content: {
                                                ...
                                                >
                                                  <SelectTrigger>
                                                    <SelectValue placeholder={`Select image for label ${index + 1}`} />
                                                  </SelectTrigger>
                                                  <SelectContent>
                                                    {(content.image_urls || []).map((img: any, imgIndex: number) => (
                                          ...
                                          <div>
                                            ...
                                            {(
                                              ...
                                              <p className="text-sm text-amber-600 mt-2">
                                                No blanks detected. Use
                                                [[answer]] format to create
                                                blanks.
                                              </p>
                                            )}
                                          </div>
                                      ...
                                      <div className="space-y-2">
                                        ...
                                        <div className="border rounded-md shadow-sm">
                                          <Tiptap
                                            ...
                                            showMathToolbar={
                                              mathToolbarStates.explanation
                                            }
                                            setShowMathToolbar={() =>
                                              toggleMathToolbar("explanation")
                                            }
                                          />
                                        </div>
                                      </div>
